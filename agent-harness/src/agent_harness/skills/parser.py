...
def parse_skill_metadata(path: Path, scope: SkillScope, qualified_prefix: str, *, trusted: bool) -> tuple[SkillRecord | None, list[SkillDiagnostic]]:
    ...
    try:
        frontmatter, _ = split_skill_file(raw)
    ...
    digest = hashlib.sha256(raw.split("\n---\n", 1)[0].encode("utf-8")).hexdigest()
    ...
    return SkillRecord(
        ...
        trusted=trusted,
        resources=_resource_manifest(path.parent),
    ), diagnostics
...
def _resource_manifest(base_dir: Path) -> tuple[SkillResource, ...]:
    ...
    for directory, kind in (("references", "reference"), ("assets", "asset"), ("scripts", "script")):
        ...
        for path in sorted(item for item in root.rglob("*") if item.is_file()):
            try:
                ...
                resources.append(SkillResource(resolved.relative_to(base_dir.resolve()).as_posix(), kind, resolved.stat().st_size))
