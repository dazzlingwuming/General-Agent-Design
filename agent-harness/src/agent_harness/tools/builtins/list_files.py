...
def create_list_files_tool(workspace_root: Path, timeout_seconds: int = 30) -> ToolDefinition:
    ...
    async def executor(args: dict[str, Any]) -> dict[str, Any]:
        ...
        return await asyncio.to_thread(_list_files_sync, workspace_root, args)
...
def _list_files_sync(workspace_root: Path, args: dict[str, Any]) -> dict[str, Any]:
    """List files on a worker thread so async callers are not blocked by disk scans."""
